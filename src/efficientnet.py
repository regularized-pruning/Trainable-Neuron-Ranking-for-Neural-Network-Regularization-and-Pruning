...
from torchvision.models._utils import _ovewrite_named_param, handle_legacy_interface, _make_divisible
...
class ConvNormActivation(torch.nn.Sequential):
    def __init__(
        ...
        conv_layer: Callable[..., torch.nn.Module] = l.MyConvLayer,
        conv_layer_bn: Callable[..., torch.nn.Module] = l.MyConvLayerBN,
        regularization = True
    ) -> None:
...
class SqueezeExcitation(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = l.MyConvLayer(input_channels=input_channels, output_channels=squeeze_channels, kernel_shape=(1,1), regularization=regularization)
        self.fc2 = l.MyConvLayer(input_channels=squeeze_channels, output_channels=input_channels, kernel_shape=(1,1), regularization=regularization)
...
class MBConv(nn.Module):
    def __init__(
        ...
        layers.append(se_layer(expanded_channels, squeeze_channels, activation=partial(nn.SiLU, inplace=True),regularization=regularization))

        # project
        layers.append(
            Conv2dNormActivation(
                expanded_channels, cnf.out_channels, kernel_size=1, norm_layer=norm_layer, activation_layer=None,regularization=regularization
            )
        )
        ...
        if expanded_channels != cnf.input_channels:
            ...
            layers.append(
                Conv2dNormActivation(
                    ...
                    stride=cnf.stride,
                    norm_layer=norm_layer,
                    activation_layer=activation_layer, regularization=regularization
                )
            )
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        layers.append(
            Conv2dNormActivation(
                3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=nn.SiLU, regularization=regularization
            )
        )
        ...
        for cnf in inverted_residual_setting:
            ...
            for _ in range(cnf.num_layers):
                ...
                stage.append(block_cnf.block(block_cnf, sd_prob, norm_layer,regularization=regularization))
                stage_block_id += 1
        ...
        self.classifier = nn.Sequential(
            ...
            l.MyLinearLayer(lastconv_output_channels, num_classes,regularization=regularization),
        )

        for m in self.modules():
            if isinstance(m, l.MyConvLayer) or  isinstance(m, l.MyConvLayerBN):
                ...
                nn.init.zeros_(m.bias)
            elif isinstance(m, l.MyLinearLayer):
                init_range = 1.0 / math.sqrt(m.size_out)
