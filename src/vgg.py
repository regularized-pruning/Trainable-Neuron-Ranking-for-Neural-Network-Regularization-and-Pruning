...
class VGG(nn.Module):
    def __init__(
        self, features: nn.Module, num_classes: int = 1000, init_weights: bool = True, dropout: float = 0.5, regularization = True
    ) -> None:
        super().__init__()
        ...
        self.classifier = nn.Sequential(
            l.MyLinearLayer(512 * 7 * 7, 4096, regularization=regularization),
            nn.ReLU(True),
            nn.Dropout(p=dropout),
            l.MyLinearLayer(4096, 4096, regularization=regularization),
            nn.ReLU(True),
            nn.Dropout(p=dropout),
            l.MyLinearLayer(4096, num_classes, regularization=regularization),
        )
        if init_weights:
...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False, regularization = True) -> nn.Sequential:
    ...
    for v in cfg:
        if v == "M":
            ...
            v = cast(int, v)
            conv2d = l.MyConvLayer(in_channels, v, kernel_shape=(3,3), padding=1, bias=True, regularization=regularization)
            if batch_norm:
                # layers += [conv2d, nn.BatchNorm2d(v), nn.ReLU(inplace=True)]
                # layers += [conv2d, l.MyBatchNormLayer(in_channels, v), nn.ReLU(inplace=True)]
                layers += [l.MyConvLayerBN(in_channels, v, kernel_shape=(3,3), padding=1, bias=True, regularization=regularization), nn.ReLU(inplace=True)]
            else:
...
def _vgg(cfg: str, batch_norm: bool, weights: Optional[WeightsEnum], progress: bool,regularization=True, **kwargs: Any) -> VGG:
    if weights is not None:
        kwargs["init_weights"] = False
        if weights.meta["categories"] is not None:
            _ovewrite_named_param(kwargs, "num_classes", len(weights.meta["categories"]))
    model = VGG(make_layers(cfgs[cfg], batch_norm=batch_norm, regularization=regularization), **kwargs)
    if weights is not None:
